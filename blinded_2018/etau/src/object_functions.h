...
double etau_analyzer::getScaleFactors(  double elept, double taupt, double eleeta, double taueta, int taudm, int tauGenMatch, bool isFakebkg)
{
  ...
  sf_taufesSF = getTauFES(TauIndex, "");
  /* if(elept<33.0)   */
  /*   {  */
  /*     e_trg_sf=1.0;  */
  /*     zptmass_weight=1.0; */
  ...
  if(elept<33.0)  e_trg_sf=1.0;
  else { e_trg24_sf=1.0; t_trg_sf=1.0; }
  sf_htt_workspace=  e_trk_sf * e_idiso_sf * e_trg24_sf * e_trg_sf * t_deepid_tightvsele_sf ;
  rv_sf = sf_htt_workspace * higgPt_weight * top_pt_weight * weight_btagSF ;
  if(elept<33.0)
    rv_sf = rv_sf *CrossTriggerSF.get_ScaleFactor(elept , eleeta);
  else if (elept>=33.0 && elept<35.0)
    rv_sf = rv_sf *TrgEle32orEle35.get_ScaleFactor(elept , eleeta);
  if(isFakebkg)
    rv_sf=rv_sf*sf_tauidSF_vvvl;
...
void etau_analyzer::jetFakeUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
   string jetFakesUnc[15] = {"rawFF_et_qcd_unc1_2018", "rawFF_et_qcd_unc2_2018", "rawFF_et_tt_unc1_2018", "rawFF_et_tt_unc2_2018", "rawFF_et_w_unc1_2018", "rawFF_et_w_unc2_2018", "FF_closure_OSSS_mvis_et_qcd_2018", "FF_closure_lpt_et_qcd", "FF_closure_lpt_et_tt", "FF_closure_lpt_et_w", "FF_closure_lpt_xtrg_et_qcd_2018", "FF_closure_lpt_xtrg_et_tt_2018", "FF_closure_lpt_xtrg_et_w_2018", "FF_closure_mt_et_w_unc1_2018", "FF_closure_mt_et_w_unc2_2018"};
  ...
  bool xtrg = false;
  if( ( HLTTau>>1&1 == 1 || HLTTau>>16&1==1 ) && my_eleP4.Pt()<33.0) xtrg=true;
...
void etau_analyzer::metSysUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  if(selected_systematic=="metresponse" && unc_shift == "up")
    fillHist(histNumber+"_CMS_htt_boson_scale_met_"+njetName+"Jet_2018_up", EleIndex, TauIndex, true, event_weight );
  else if(selected_systematic=="metresolution" && unc_shift == "up")
    fillHist(histNumber+"_CMS_htt_boson_reso_met_"+njetName+"Jet_2018_up", EleIndex, TauIndex, true, event_weight );
  
  else if(selected_systematic=="metresponse" && unc_shift == "down")
    fillHist(histNumber+"_CMS_htt_boson_scale_met_"+njetName+"Jet_2018_down", EleIndex, TauIndex, true, event_weight );
  else if(selected_systematic=="metresolution" && unc_shift == "down")
    fillHist(histNumber+"_CMS_htt_boson_reso_met_"+njetName+"Jet_2018_down", EleIndex, TauIndex, true, event_weight );
...
void etau_analyzer::metClusteredUnc( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  if(selected_systematic=="metunclustered" && unc_shift == "up")
    fillHist(histNumber+"_CMS_scale_met_unclustered_2018_up", EleIndex, TauIndex, true, event_weight);
  if(selected_systematic=="metunclustered" && unc_shift == "down")
    fillHist(histNumber+"_CMS_scale_met_unclustered_2018_down", EleIndex, TauIndex, true, event_weight);
...
TLorentzVector etau_analyzer::applyTauESCorrections(TLorentzVector tauP4, int tauIndex, int shift)
{
  ...
  int taudm = tau_DecayMode->at(tauIndex);
  if (tauGenmatching>=5) tauP4Corr = tauP4 * get_BinContent(h_tauesSF, taudm);
  /* if (tauGenmatching>=5 && tau_DecayMode->at(tauIndex)==0) tauP4Corr=tauP4*0.987; */
  /* else if (tauGenmatching>=5 && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4*0.995; */
  /* else if (tauGenmatching>=5 && tau_DecayMode->at(tauIndex)==10) tauP4Corr=tauP4*0.988; */
  /* if (  (tauGenmatching==1 || tauGenmatching==3) && tau_DecayMode->at(tauIndex)==0 ) tauP4Corr=tauP4*0.968; */
  /* else if ( (tauGenmatching==1 || tauGenmatching==3) && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4*1.026; */
  /* if (  (tauGenmatching==2 || tauGenmatching==4) && tau_DecayMode->at(tauIndex)==0 ) tauP4Corr=tauP4*0.998; */
  /* else if ( (tauGenmatching==2 || tauGenmatching==4) && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4*0.990; */
