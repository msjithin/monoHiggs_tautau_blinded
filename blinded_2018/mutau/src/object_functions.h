...
double mutau_analyzer::getScaleFactors(  double mupt, double taupt, double mueta, double taueta, int taudm, int tauGenMatch, bool isFakebkg)
{
  ...
  if (sf_muTrg == 0 ) sf_muTrg =1.0;
  //if(  tauGenMatch>=5 )
    {
      // sf_tauidSF_m = min(get_BinContent(h_tauidSF_m, taudm) , 1.0);
      // sf_tauidSF_vvvl = get_BinContent(h_tauidSF_vvvl, taudm);
      //cout <<" tau id sf "<<fn_tauIDSF_m->Eval(taupt)<<endl;
  ...
  //   if(abs(taueta) < 0.4 ) sf_fakeMu=1.28;
  //   if(abs(taueta) > 0.4 && abs(taueta) < 0.8 ) sf_fakeMu=1.2;
  //   if(abs(taueta) > 0.8 && abs(taueta) < 1.2 ) sf_fakeMu=1.08;
  //   if(abs(taueta) > 1.2 && abs(taueta) < 1.7 ) sf_fakeMu=1.0;
  //   if(abs(taueta) > 1.7 && abs(taueta) < 2.3 ) sf_fakeMu=2.3;
  // }
  ...
  // else if(  my_muP4.Pt() >= 28.0 )
  //   rv_sf = rv_sf*IsoMu27SF.get_ScaleFactor(my_muP4.Pt(), my_muP4.Eta());

  if(isFakebkg)
    rv_sf = rv_sf * sf_tauidSF_vvvl;

  //if( rv_sf > 1.1 && mupt < 50 )
    // {
    //   printTabSeparated("rv_sf = ", rv_sf);
    //   printTabSeparated("inputs", mupt, taupt, taudm, " isfake=", isFakebkg);
    //   printTabSeparated("row 1", m_tracking , m_IDiso ,  mu_singletrg , mu_crosstrg , tau_crossttrg , t_deepid , higgPt_weight , top_pt_weight , weight_btagSF);
    //   printTabSeparated("row 2", sf_muID , sf_IsoEff ,sf_muTrg,  sf_tauTrg , sf_fakeMu, sf_fakeEle , sf_htt_workspace );
    //   printTabSeparated("row 2a", sf_tauidSF_m, sf_tauTrg, sf_fakeMu);
    //   printTabSeparated("row 3", MuonIDIso.get_ScaleFactor(my_muP4.Pt(), my_muP4.Eta()), CrossTriggerSF.get_ScaleFactor(my_muP4.Pt(), my_muP4.Eta()), IsoMu24or27SF.get_ScaleFactor(my_muP4.Pt(), my_muP4.Eta()), IsoMu27SF.get_ScaleFactor(my_muP4.Pt(), my_muP4.Eta()) );
    //   printTabSeparated("row 4", sf_tauidSF_vvvl );
    //   cout<<""<<endl;
    // }
...
TLorentzVector mutau_analyzer::applyTauESCorrections(TLorentzVector tauP4, int tauIndex, int shift)
{
  ...
  if ( tauGenmatching>=5 && tau_DecayMode->at(tauIndex)==0) tauP4Corr=tauP4Corr*0.987;
  else if ( tauGenmatching>=5 && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4Corr*0.995;
  else if ( tauGenmatching>=5 && tau_DecayMode->at(tauIndex)==10) tauP4Corr=tauP4Corr*0.988;
  if ( (tauGenmatching==1 || tauGenmatching==3) && tau_DecayMode->at(tauIndex)==0) tauP4Corr=tauP4Corr*0.968;
  else if ( (tauGenmatching==1 || tauGenmatching==3) && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4Corr*1.026;
  if ( (tauGenmatching==2 || tauGenmatching==4) && tau_DecayMode->at(tauIndex)==0) tauP4Corr=tauP4Corr*0.998;
  else if ( (tauGenmatching==2 || tauGenmatching==4) && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4Corr*0.990;
