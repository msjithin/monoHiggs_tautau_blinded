...
//$ ./rootcom skimm_signal_2018 analyze
//
...
//
#define skimm_signal_2018_cxx
#include "skimm_signal_2018.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[])
{
  ...
  skimm_signal_2018 t(argv[1],argv[2], isMC);
...
void skimm_signal_2018::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
...
void skimm_signal_2018::BookHistos(const char* file2)
{
...
void skimm_signal_2018::fillHistos(int histoNumber, double event_weight, int higgs_Index)
{
...
bool skimm_signal_2018::skimming_Htt(){
  bool tmpCand=false;
...
std::vector<int> skimm_signal_2018::skimmed_Ele(){
...
std::vector<int> skimm_signal_2018::skimmed_Tau(){
  std::vector<int> tmpCand;    tmpCand.clear();
  for(int iTau=0; iTau<nTau;iTau++){
    if( tau_Pt->at(iTau) > 29.5           &&
	fabs( tau_Eta->at(iTau))< 2.3     &&
...
void skimm_signal_2018::fillOutTree(){
  run_ = run;
  event_= event;
  lumis_=lumis;
  tree->Fill();
...
double skimm_signal_2018::dR(int ele_index, int tau_index)
{
...
double skimm_signal_2018::DeltaPhi(double phi1, double phi2)
