...
void tautau_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
      ...
      if(metFilters==0 && passTauTrigger)
	{
	  ...
	  if(is_MC){
	    // // /// UP 
	    selections(event_weight,  1, "tau1ES");
	    selections(event_weight,  1, "tau2ES");
	    ...
	    selections(event_weight,  1, "tau1IDunc");
	    selections(event_weight,  1, "tau2IDunc");
	    selections(event_weight,  1, "tau1TRGunc");
	    selections(event_weight,  1, "tau2TRGunc");
