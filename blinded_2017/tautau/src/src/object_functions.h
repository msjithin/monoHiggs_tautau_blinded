...
double tautau_analyzer::getScaleFactors(  double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  // sf_tauTrg = tauTrgsf_m->getSF(my_tau1P4.Pt(), tau1dm,0)*tauTrgsf_m->getSF(my_tau2P4.Pt(), tau2dm,0);
  // sf_tauTrg_vvvloose= tauTrgsf_vl->getSF(my_tau1P4.Pt(), tau1dm,0);
...
void tautau_analyzer::jetFakeUnc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  string uncNames[3] = {"CMS_rawFF_tt_qcd_2017", "CMS_FF_closure_tau2pt_tt_qcd", "CMS_FF_closure_tt_qcd_osss"};
...
void tautau_analyzer::tauIDunc( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  if(selected_systematic=="tau1IDunc")
	...
	string uncName = "CMS_tauideff_t1_" + tauDM_map[i].first+"_2017";
  ...
  if (selected_systematic=="tau2IDunc")
	...
	string uncName = "CMS_tauideff_t2_" + tauDM_map[i].first+"_2017";
...
void tautau_analyzer::dyShape( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
   if(selected_systematic!="dyShape")
    return;
  if(unc_shift == "up")
    fillHist(histNumber+"_CMS_htt_dyShape_up", Tau1Index, Tau2Index, true, event_weight * 1.1);
  else if (unc_shift == "down")
    fillHist(histNumber+"_CMS_htt_dyShape_down", Tau1Index, Tau2Index, true, event_weight * 0.9);
}
